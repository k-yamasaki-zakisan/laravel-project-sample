...
<div class="row">
    <div class="col-md-12">
        <div class="card card-info">
            ...
            <form action={{ route('exhibitor_admin.exhibitor.images.update_sort', $slug) }} method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <ul class="todo-list image-list" data-widget="image-list" id="add-image-zone">

                        @foreach($exhibitor_images as $exhibitor_image)
                        <li>
                            <input type="hidden" name="sort_indexs[]" value="{{ $exhibitor_image['id'] }}">
                            <!-- drag handle -->
                            ...
                            <!-- image -->
                            <img src="{{ asset('storage/' . $exhibitor_image['image_path']) }}" width="50%" />
                            <div class="tools">
                                <i class="fa fa-trash js-ajax-deleteButton image-{{ $exhibitor_image['id'] }}"
                                    data-target="image-{{ $exhibitor_image['id'] }}" data-url="{{ route('exhibitor_admin.exhibitor.images.destroy', [
                                        'expo_slug' => $slug,
                                        'id' => $exhibitor_image['id']
                                    ]) }}"></i>
                            </div>
                        </li>
                        @endforeach
...
url: "{{ route('exhibitor_admin.exhibitor.images.store', $slug) }}", // Set the url
method: "post",
thumbnailWidth: 80,
thumbnailHeight: 80,
parallelUploads: 20,
paramName: "exhibitor_image",
...
imageListItem.find('img').attr('src', "{{ asset('storage') }}" + '/' + result['exhibitor_image']['image_path']);
imageListItem.find('input').attr('value', result['exhibitor_image']['id']);
// 動画削除path作成
const deletePath = "{{ route('exhibitor_admin.exhibitor.images.store', [$slug]) }}/" + result['exhibitor_image']['id'];
imageListItem.find('.js-ajax-deleteButton').attr('data-url', deletePath);
imageListItem.find('.js-ajax-deleteButton').attr('data-target', "image-"+ result['exhibitor_image']['id']);
imageListItem.find('.js-ajax-deleteButton').addClass("image-" + result['exhibitor_image']['id']);
...
<!-- end of exhibitor_admin.exhibitors.edit.images -->s